...
export default function Home() {
  ...
  // URL pointing to a generated video preview. When set, a <video> element
  // is rendered to allow in‑browser playback of the generated story.  The
  // value is cleared whenever a new story is generated or when the user
  // navigates away from the preview.  See previewVideo() for usage.
  ...
  const generateStory = async () => {
    ...
    setError(null);
    setStory(null);
    // Clear any previous video preview when generating a new story
    setVideoUrl(null);
    try {
  ...
  const rerollImage = async (idx: number) => {
    ...
    try {
      ...
      const updatedPages = story.pages.map((p, i) => (i === idx ? { ...p, imageUrl: url } : p));
  ...
  const rerollText = async (idx: number) => {
    ...
    try {
      ...
      const updatedPages = story.pages.map((p, i) => (i === idx ? { ...p, text: newText } : p));
      setStory({ ...story, pages: updatedPages });
    } catch (err: any) {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      const pages = Array.from(bookRef.current.querySelectorAll(
        ".story-page",
      )) as HTMLElement[];
  ...
  const downloadVideo = async () => {
    ...
    try {
      ...
      const pageEls = Array.from(bookRef.current.querySelectorAll(
        ".story-page",
      )) as HTMLElement[];
      ...
      // To avoid network failures when loading the FFmpeg core, we load the
      // JavaScript and WASM modules from the `/public/ffmpeg` folder.  You
      // must copy `ffmpeg-core.js` and `ffmpeg-core.wasm` from
      // `node_modules/@ffmpeg/core/dist` into `public/ffmpeg` in your project.
      await ffmpeg.load({
        coreURL: "/ffmpeg/ffmpeg-core.js",
        wasmURL: "/ffmpeg/ffmpeg-core.wasm",
        // The worker script is required by ffmpeg.wasm to run in a Web Worker.
        // Copy `ffmpeg-core.worker.js` from `node_modules/@ffmpeg/core/dist/umd`
        // into your `public/ffmpeg` folder and reference it here.
      ...
      // 5) Build segments with fades.  We omit the page-turn sound effect to
      // avoid fetching external audio files.  Each segment consists of a still
      // image that fades out and the corresponding narration audio.
      ...
      await ffmpeg.writeFile("list.txt", new TextEncoder().encode(listFile.join("\n")));
      await ffmpeg.exec([
        "-f",
        "concat",
        "-safe",
  ...
  /**
   * Generate a narrated video and set it as a preview.  This shares the
   * underlying logic with downloadVideo() but instead of forcing a file
   * download, it assigns the resulting MP4 to the videoUrl state.  The
   * caller can then render a <video> element pointing to the URL to play
  ...
  const previewVideo = async () => {
    ...
    try {
      ...
      // 3) Load FFmpeg wasm using local core and worker files
      const ffmpeg = new FFmpeg();
      await ffmpeg.load({
        coreURL: "/ffmpeg/ffmpeg-core.js",
        wasmURL: "/ffmpeg/ffmpeg-core.wasm",
      ...
      // 5) Build segments with fades (no page-turn sound effect)
      const listFile: string[] = ["ffconcat version 1.0"];
      for (let i = 0; i < pagesWithAudio.length; i++) {
        const imageName = images[i].name;
        const outName = `seg${i}.mp4`;
      ...
      // Set the generated video URL for in-browser playback
      setVideoUrl(url);
    } catch (err: any) {
      alert(err.message || "Failed to generate preview video");
    } finally {
      setBusy(false);
    }
  };
      ...
      {story && (
        <>
          <div style={{ marginTop: 16, display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button onClick={downloadVideo} disabled={busy}>Export Video</button>
            <button onClick={previewVideo} disabled={busy}>Preview Video</button>
          </div>
          ...
          {/* Show a video preview if one has been generated */}
          {videoUrl && (
